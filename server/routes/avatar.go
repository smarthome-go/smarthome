...
func handleAvatarUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(int64(maxUploadSize)); err != nil {
		...
		api.Res(w, api.Response{Success: false, Message: "failed to upload avatar", Error: "could not parse form"})
		return
	}
	...
	if err != nil {
		...
		api.Res(w, api.Response{Success: false, Message: "failed to obtain file", Error: "failed to obtain file: could not get file from request"})
		return
	}
	defer file.Close()
	...
	if handler.Size > int64(maxUploadSize) {
		...
		api.Res(w, api.Response{Success: false, Message: "file too large", Error: "could not use file: filesize too large"})
		return
	}
	...
	if !fileEndingValid {
		...
		api.Res(w, api.Response{Success: false, Message: "avatar upload failed", Error: "invalid file type. allowed types are: [png / webp / jpeg / jpg]"})
		return
	}
	// Do the actual setup
	if err := user.UploadAvatar(username, handler.Filename, file); err != nil {
		...
		w.WriteHeader(http.StatusInternalServerError)
		api.Res(w, api.Response{Success: false, Message: "avatar upload failed", Error: "internal server error"})
		return
	}
	api.Res(w, api.Response{Success: true, Message: "avatar uploaded successfully"})
}
...
func deleteAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		api.Res(w, api.Response{Success: false, Message: "avatar removal failed", Error: "database error"})
		return
	}
	// Check if the user has a custom avatar
	if filepathBefore == "./web/assets/avatar/default.png" {
		w.WriteHeader(http.StatusUnprocessableEntity)
		api.Res(w, api.Response{Success: false, Message: "avatar removal failed", Error: "the default avatar cannot be removed"})
		return
	}
	if err := user.RemoveAvatar(username); err != nil {
		log.Error("Could remove avatar: ", err.Error())
		w.WriteHeader(http.StatusInternalServerError)
		api.Res(w, api.Response{Success: false, Message: "avatar removal failed", Error: "internal server error"})
		return
	}
	api.Res(w, api.Response{Success: true, Message: "avatar removed successfully"})
}
